...
def extract_mov_creation_date(file_path: str) -> Optional[str]:
    ...
    try:
        ...
        if not parser:
            logger.warning("Could not create parser for MOV file: %s", file_path)
            return None
        ...
        logger.debug("No creation date found in MOV file: %s", file_path)
        return None

    except (OSError, IOError, ValueError) as e:
        logger.error("Error extracting MOV creation date from %s: %s", file_path, e)
        return None
